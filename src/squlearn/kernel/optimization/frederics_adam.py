# This is just a small variation of the Qiskit ADAM implementation
# it is modified to enable the plot of the loss function over the iterations
...
class ADAM(Optimizer):
    ...
    def save_params(self, snapshot_dir: str) -> None:
        ...
        if self._amsgrad:
            ...
            with open(os.path.join(snapshot_dir, "adam_params.csv"), mode="a") as csv_file:
                fieldnames = ["v", "m", "t", "loss"]
                writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
                writer.writerow({"v": self._v, "m": self._m, "t": self._t, "loss": self._loss})
    ...
    def load_params(self, load_dir: str) -> None:
        ...
        self._loss = np.fromstring(loss, dtype=int, sep=",")
    ...
    def minimize(
        ...
        while self._t < self._maxiter:
            ...
            lr_eff = (
                self._lr * np.sqrt(1 - self._beta_2**self._t) / (1 - self._beta_1**self._t)
            )
            if not self._amsgrad:
                params_new = params - lr_eff * self._m.flatten() / (
                    np.sqrt(self._v.flatten()) + self._noise_factor
                )
                self._loss[self._t - 1] = fun(params_new)
            else:
            ...
            # if np.linalg.norm(params - params_new) < self._tol and self._t > 2:
            #     break
