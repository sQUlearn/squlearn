...
def decompose_to_std(circuit: QuantumCircuit, gate_list: list = []) -> QuantumCircuit:
    ...
    Args:
        ...
        gate_list (list): List of gates, which are considered as standard gates. If the gate is
            not in the list, it will be decomposed. If the list is empty, all gates from
            `qiskit.circuit.library.standard_gates` and `["cx","cy","cz","measure"]` are considered
            as standard gates.
