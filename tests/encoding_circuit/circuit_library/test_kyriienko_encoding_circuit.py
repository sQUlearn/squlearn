...
class TestKyriienkoEncodingCircuit:
    ...
    def test_kyriienko_get_circuit_ground_truth(
        ...
        def build_expected_kyriienko_circuit(
            num_qubits: int,
            num_features: int,
            encoding_style: str,
            ...
            if encoding_style == "chebyshev_product":
                QC_layered = LayeredEncodingCircuit(
                    num_qubits=num_qubits, num_features=num_features
                )
                layer = Layer(QC_layered)
                # mapping in product branch: mapping layer.Rx/Ry/Rz ("x" name) with encoding=np.arcsin
                {"rx": layer.Rx, "ry": layer.Ry, "rz": layer.Rz}[rotation_gate](
                    ...
                    for outer_ in range(outer):
                        for inner_ in range(inner):
                            angle = (
                                alpha
                                * icheb
                                * np.arccos(features[index_offset_encoding % num_features])
                            )
                            # pick rotation gate
                            if rotation_gate == "rx":
        ...
        qc_expected = build_expected_kyriienko_circuit(
            num_qubits=num_qubits,
            num_features=num_features,
            encoding_style=encoding_style,
            variational_arrangement=variational_arrangement,
